...
class ImportTests(TestCase):
    ...
    def test_upload_csv_with_invalid_file(self):
        ...
        response = self.client.post(reverse("import:process"), {"file": "invalid"}, follow=True)
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Unable to process the uploaded file.")
    ...
    def test_upload_csv_with_empty_file(self):
        ...
        response = self.client.post(reverse("import:process"), {"file": file}, follow=True)
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Unable to process the uploaded file.")
    ...
    def test_save_with_valid_data(self):
        ...
        self.assertEqual(trip.start, datetime(2022, 6, 17, 11, 0, 0, tzinfo=ZoneInfo("UTC")))
        self.assertEqual(trip.end, datetime(2022, 6, 17, 17, 0, 0, tzinfo=ZoneInfo("UTC")))
        ...
        self.assertEqual(trip.start, datetime(2022, 6, 20, 15, 0, 0, tzinfo=ZoneInfo("UTC")))
