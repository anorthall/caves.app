...
class SocialFeedTests(TestCase):
    ...
    def test_feed_ordering_by_trip_start(self):
        ...
        self.client.force_login(self.user)
        response = self.client.post(reverse("log:feed_set_ordering"), {"sort": User.FEED_DATE})
    ...
    def test_feed_ordering_by_trip_added(self):
        ...
        self.client.force_login(self.user)
        response = self.client.post(reverse("log:feed_set_ordering"), {"sort": User.FEED_ADDED})
        self.assertEqual(response.status_code, 302)
        self.assertEqual(response.url, reverse("log:index"))
    ...
    def test_setting_feed_ordering_to_invalid_value(self):
        """Test that the feed ordering is not changed if an invalid value is provided."""
        self.user.feed_ordering = User.FEED_DATE
        self.user.save()

        self.client.force_login(self.user)
        response = self.client.post(reverse("log:feed_set_ordering"), {"sort": "invalid"})
        self.assertEqual(response.status_code, 302)
        self.assertEqual(response.url, reverse("log:index"))
    ...
    def test_get_trips_context_with_a_user_without_trips(self):
        ...
        user = User.objects.create_user(email="test@user.app", username="test", name="Test User")
        request = MagicMock()
        request.user = user
