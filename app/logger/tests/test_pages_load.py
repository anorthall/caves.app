...
class TestLoggerPagesLoad(TestCase):
    def setUp(self):
        ...
        self.trip = Trip.objects.create(user=self.user, cave_name="Test Trip", start=timezone.now())
    ...
    def test_htmx_trip_like_toggle_page_loads(self):
        ...
        self.client.force_login(self.user)
        response = self.client.post(reverse("log:trip_like_htmx_view", args=[self.trip.uuid]))
        self.assertEqual(response.status_code, 200)
